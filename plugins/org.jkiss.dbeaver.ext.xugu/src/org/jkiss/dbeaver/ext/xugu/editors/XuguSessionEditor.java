...
import org.jkiss.dbeaver.ext.xugu.model.session.XuguServerSession;
 ...
 * OracleSessionEditor
 */
public class XuguSessionEditor extends AbstractSessionEditor
{
    ...
    protected SessionManagerViewer createSessionViewer(DBCExecutionContext executionContext, Composite parent) {
        return new SessionManagerViewer<XuguServerSession>(this, parent, new XuguServerSessionManager(getExecutionContext())) {
                ...
                contributionManager.add(ActionUtils.makeActionContribution(
                    new Action("Show background", Action.AS_CHECK_BOX) {
                        ...
                        public void run() {
                            ...
                            refreshPart(XuguSessionEditor.this, true);
                        }
                    }, true));
            ...
            public Map<String, Object> getSessionOptions() {
                ...
                if (showBackground) {
                    options.put(XuguServerSessionManager.OPTION_SHOW_BACKGROUND, true);
                }
                if (showInactive) {
                    options.put(XuguServerSessionManager.OPTION_SHOW_INACTIVE, true);
                }
                return options;
            }
    ...
    private class DisconnectSessionAction extends Action {
        ...
        {
            super(
                kill ? XuguMessages.editors_oracle_session_editor_title_kill_session : XuguMessages.editors_oracle_session_editor_title_disconnect_session,
        ...
        public void run()
        {
            ...
            final String action = (kill ? XuguMessages.editors_oracle_session_editor_action_kill : XuguMessages.editors_oracle_session_editor_action_disconnect) + XuguMessages.editors_oracle_session_editor_action__session;
            ConfirmationDialog dialog = new ConfirmationDialog(
                getSite().getShell(),
                action,
                null,
                NLS.bind(XuguMessages.editors_oracle_session_editor_confirm_action, action.toLowerCase(), sessions),
                MessageDialog.CONFIRM,
                new String[] { IDialogConstants.YES_LABEL, IDialogConstants.NO_LABEL },
                0,
                XuguMessages.editors_oracle_session_editor_confirm_title,
                false);
            if (dialog.open() == IDialogConstants.YES_ID) {
                Map<String, Object> options = new HashMap<>();
                if (kill) {
                    options.put(XuguServerSessionManager.PROP_KILL_SESSION, kill);
                }
                if (dialog.getToggleState()) {
                    options.put(XuguServerSessionManager.PROP_IMMEDIATE, true);
                }

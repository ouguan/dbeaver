 ...
 */
public class XuguCompilerDialog extends BaseDialog
{
    private static final Log log = Log.getLog(XuguCompilerDialog.class);

    private static final int COMPILE_ID = 1000;
    private static final int COMPILE_ALL_ID = 1001;

    private java.util.List<XuguSourceObject> compileUnits;
    private TableViewer unitTable;

    private ObjectCompilerLogViewer compileLog;


    public XuguCompilerDialog(Shell shell, java.util.List<XuguSourceObject> compileUnits)
    {
        super(shell, XuguMessages.views_oracle_compiler_dialog_title, null);
    ...
    protected Composite createDialogArea(Composite parent)
    {
...

        {
            ...
            columnController.addColumn(XuguMessages.views_oracle_compiler_dialog_column_name, null, SWT.NONE, true, true, new CellLabelProvider() {
                @Override
            ...
            columnController.addColumn(XuguMessages.views_oracle_compiler_dialog_column_type, null, SWT.NONE, true, true, new CellLabelProvider() {
                @Override
                public void update(ViewerCell cell)
                {
                    DBSObject unit = (DBSObject) cell.getElement();
            ...
            unitTable.addDoubleClickListener(event -> {
                ...
                if (!selection.isEmpty()) {
                    XuguSourceObject unit = (XuguSourceObject) selection.getFirstElement();
    ...
    protected void createButtonsForButtonBar(Composite parent)
    {
        ...
        createButton(parent, COMPILE_ID, XuguMessages.views_oracle_compiler_dialog_button_compile, false).setEnabled(false);
		createButton(parent, COMPILE_ALL_ID, XuguMessages.views_oracle_compiler_dialog_button_compile_all, true);
    ...
    {
        final List<XuguSourceObject> toCompile;
    ...
    private void performCompilation(DBRProgressMonitor monitor, List<XuguSourceObject> units)
    {
        compileLog.layoutLog();
        for (XuguSourceObject unit : units) {
            if (monitor.isCanceled()) {
                break;
            }
            final String message = NLS.bind(XuguMessages.views_oracle_compiler_dialog_message_compile_unit, unit.getSourceType().name(), unit.getName());
            ...
            compileLog.info(!success ? XuguMessages.views_oracle_compiler_dialog_message_compilation_error : XuguMessages.views_oracle_compiler_dialog_message_compilation_success);

...
import org.jkiss.dbeaver.ext.xugu.model.dict.XuguLanguage;
import org.jkiss.dbeaver.ext.xugu.model.dict.XuguTerritory;
 ...
 */
public class XuguConnectionExtraPage extends ConnectionPageAbstract
{
    ...
    public XuguConnectionExtraPage()
    {
    ...
    public void createControl(Composite parent)
    {
...

        {
            ...
            languageCombo.add(XuguConstants.NLS_DEFAULT_VALUE);
            for (XuguLanguage language : XuguLanguage.values()) {
                languageCombo.add(language.getLanguage());
            }
            languageCombo.setText(XuguConstants.NLS_DEFAULT_VALUE);
            ...
            territoryCombo.add(XuguConstants.NLS_DEFAULT_VALUE);
            for (XuguTerritory territory : XuguTerritory.values()) {
                territoryCombo.add(territory.getTerritory());
            }
            territoryCombo.setText(XuguConstants.NLS_DEFAULT_VALUE);
    ...
    public void loadSettings()
    {
        ...
        // Settings
        final Object nlsLanguage = providerProperties.get(XuguConstants.PROP_SESSION_LANGUAGE);
        if (nlsLanguage != null) {
            languageCombo.setText(nlsLanguage.toString());
        }

        final Object nlsTerritory = providerProperties.get(XuguConstants.PROP_SESSION_TERRITORY);
        if (nlsTerritory != null) {
            territoryCombo.setText(nlsTerritory.toString());
        }

        final Object dateFormat = providerProperties.get(XuguConstants.PROP_SESSION_NLS_DATE_FORMAT);
        if (dateFormat != null) {
            nlsDateFormat.setText(dateFormat.toString());
        }

        final Object checkSchemaContent = providerProperties.get(XuguConstants.PROP_CHECK_SCHEMA_CONTENT);
        if (checkSchemaContent != null) {
            hideEmptySchemasCheckbox.setSelection(CommonUtils.getBoolean(checkSchemaContent, false));
        }

        showDBAAlwaysCheckbox.setSelection(CommonUtils.getBoolean(providerProperties.get(XuguConstants.PROP_ALWAYS_SHOW_DBA), false));
        useDBAViewsCheckbox.setSelection(CommonUtils.getBoolean(providerProperties.get(XuguConstants.PROP_ALWAYS_USE_DBA_VIEWS), false));
        useRuleHint.setSelection(CommonUtils.getBoolean(providerProperties.get(XuguConstants.PROP_USE_RULE_HINT), false));
    }
    ...
    public void saveSettings(DBPDataSourceContainer dataSource)
    {
        ...
        {
            // Settings
            if (!XuguConstants.NLS_DEFAULT_VALUE.equals(languageCombo.getText())) {
                providerProperties.put(XuguConstants.PROP_SESSION_LANGUAGE, languageCombo.getText());
            } else {
                providerProperties.remove(XuguConstants.PROP_SESSION_LANGUAGE);
            }

            if (!XuguConstants.NLS_DEFAULT_VALUE.equals(territoryCombo.getText())) {
                providerProperties.put(XuguConstants.PROP_SESSION_TERRITORY, territoryCombo.getText());
            } else {
                providerProperties.remove(XuguConstants.PROP_SESSION_TERRITORY);
            }

            String dateFormat = nlsDateFormat.getText();
            if (!dateFormat.isEmpty()) {
                providerProperties.put(XuguConstants.PROP_SESSION_NLS_DATE_FORMAT, dateFormat);
            } else {
                providerProperties.remove(XuguConstants.PROP_SESSION_NLS_DATE_FORMAT);
            }

            providerProperties.put(
                XuguConstants.PROP_CHECK_SCHEMA_CONTENT,
                String.valueOf(hideEmptySchemasCheckbox.getSelection()));
            providerProperties.put(
                XuguConstants.PROP_ALWAYS_SHOW_DBA,
                String.valueOf(showDBAAlwaysCheckbox.getSelection()));
            providerProperties.put(
                XuguConstants.PROP_ALWAYS_USE_DBA_VIEWS,
                String.valueOf(useDBAViewsCheckbox.getSelection()));

            providerProperties.put(
                XuguConstants.PROP_USE_RULE_HINT,

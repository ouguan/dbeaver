...
public class PackageNavigateHandler extends AbstractHandler //implements IElementUpdater
{
    ...
    {
        final XuguProcedurePackaged procedure = getSelectedProcedure(event);
        if (procedure != null) {
            XuguPackage procPackage = procedure.getParentObject();
    ...
    static class NavigateJob extends AbstractJob {
        ...
        private final XuguProcedurePackaged procedure;
        private final SQLEditorBase sqlEditor;

        public NavigateJob(XuguProcedurePackaged procedure, SQLEditorBase sqlEditor) {
        ...
        private void navigate(DBRProgressMonitor monitor) throws InterruptedException, DBException {
            ...
            if (document != null) {
                ...
                final Collection<XuguProcedureArgument> parameters = procedure.getParameters(monitor);
                if (parameters != null) {
    ...
    private XuguProcedurePackaged getSelectedProcedure(ExecutionEvent event)
    {
        final ISelection currentSelection = HandlerUtil.getCurrentSelection(event);
        if (currentSelection instanceof IStructuredSelection && !currentSelection.isEmpty()) {
            Object firstElement = ((IStructuredSelection) currentSelection).getFirstElement();
            return RuntimeUtils.getObjectAdapter(firstElement, XuguProcedurePackaged.class);
        }
        return null;
    }
